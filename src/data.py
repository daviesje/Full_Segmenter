...
def load_image_train(file_path):
    ...
    input_image = tf.image.resize(img, constants.img_size)

    return input_image


def load_mask_train(mask_path):
    mask = tf.io.read_file(mask_path)
    mask = tf.image.decode_png(mask, channels=constants.mask_channels)

    input_mask = tf.image.resize(mask, constants.img_size)
    input_mask = ceildiv(tf.cast(input_mask, tf.int32),255)

    input_mask = tf.one_hot(tf.squeeze(input_mask), constants.n_labels)
...
def load_images(data_dir, img_size=(128, 128), img_channels=3, mask_channels=1, n_labels=2):
    ...
    # file list -> dataset
    x_train = tf.data.Dataset.list_files(str(data_dir / 'train_frames/*.png'),shuffle=False)
    x_test = tf.data.Dataset.list_files(str(data_dir / 'test_frames/*.png'),shuffle=False)
    y_train = tf.data.Dataset.list_files(str(data_dir / 'train_masks/*.png'),shuffle=False)
    y_test = tf.data.Dataset.list_files(str(data_dir / 'test_masks/*.png'),shuffle=False)


    # list dataset -> image dataset
    x_train = x_train.map(load_image_train, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    y_train = y_train.map(load_mask_train, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    x_test = x_test.map(load_image_test, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    y_test = y_test.map(load_mask_test, num_parallel_calls=tf.data.experimental.AUTOTUNE)

    train = tf.data.Dataset.zip((x_train, y_train))
    test = tf.data.Dataset.zip((x_test, y_test))

    info = {}
    image_count = len(list(data_dir.glob('train_frames/*.png')))
    info['train_count'] = image_count
    test_count = len(list(data_dir.glob('test_frames/*.png')))
